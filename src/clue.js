...
function selectRandom(array){
  const randomNum = Math.floor(Math.random() * array.length) //gives a random number between 0 and length of array 
  return array[randomNum]; // returns the random element inside the array in the selectRandom function 
  }


//Math.floor(Math.random() * (max - min) + min --> max 5 min 0
...
//    let guessedSuspect = document.querySelector('#suspect-options').value
//    let guessedWeapon = document.querySelector('#weapons-options').value
//    let guessedRoom = document.querySelector('#room-options').value
